...
| 7 · Frases naturales, «qué dato» y calibración | Hecha | Evaluación de 64 a 134 frases en grupos (natural, jerga, erratas, dato, excepciones, fuera de ámbito) con varias respuestas válidas por frase. Nueva pregunta a Jev «qué dato» (cantidad, valor, precio, proveedor, formatos, mínimo): la respuesta enseña solo eso (sin € si se pregunta cuántas hay; «¿a cómo sale X?» da el último precio, no la ficha). Proveedores nombrados en el mensaje van en el state («ha llegado lo de Distribuciones Canarias» → recibir pedido). Calibración en la evaluación (ECE, fiabilidad por tramo y umbral recomendado por decisión y contexto) y umbrales ajustados con ella. Textos más cortos. |

## Cómo se mide cada etapa
- `npm run copiloto:eval` (Jev real): aciertos por grupo de frases y calibración de cada decisión en `docs/copiloto/EVALUACION.md`; `-- --grupo jerga` para mirar solo un grupo.
