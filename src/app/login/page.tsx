...
export default function LoginPage() {
  ...
  return <main className="login-page"><div className="login-art"><div className="login-brand"><span className="login-mark"><Sparkles size={18} /></span><strong>nexo<span>.</span></strong></div><div className="art-copy"><p>Inventario claro.<br /><em>Decisiones rápidas.</em></p><span>Todo tu equipo, todos tus locales, en el mismo pulso.</span></div><div className="art-orbit orbit-one" /><div className="art-orbit orbit-two" /><div className="art-sticker">4 locales<br /><b>· conectados</b></div></div><section className="login-form-wrap"><div className="login-form-box"><div className="mobile-login-brand"><span className="login-mark"><Sparkles size={18} /></span><strong>nexo<span>.</span></strong></div><p className="login-kicker">Bienvenido de nuevo</p><h1>Tu inventario,<br /><span>en orden.</span></h1><p className="login-subtitle">Accede al espacio de trabajo de tu equipo.</p><form onSubmit={submit}><label>Email<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="tu@email.com" required /><Mail size={16} /></label><label>Contraseña<div className="password-wrap"><input type={showPassword ? "text" : "password"} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="••••••••" required /><button type="button" onClick={() => setShowPassword(!showPassword)} aria-label={showPassword ? "Ocultar contraseña" : "Mostrar contraseña"}>{showPassword ? <EyeOff size={16} /> : <Eye size={16} />}</button></div></label><div className="login-options"><label className="check-label"><input type="checkbox" /> <span>Recordarme</span></label><button type="button" className="forgot-button">¿Has olvidado la contraseña?</button></div>{message && <p className="login-message">{message}</p>}<button className="login-submit" disabled={loading}>{loading ? "Accediendo..." : "Entrar al espacio"}<ArrowRight size={17} /></button></form><div className="login-divider"><span>o entra sin contraseña</span></div><button className="magic-button" onClick={magicLink} disabled={loading}><LockKeyhole size={16} /> Enviarme un enlace mágico</button><p className="login-legal">Al continuar aceptas los términos de uso y la política de privacidad.</p></div></section></main>;
}
